...
regularizer = tf.contrib.layers.l2_regularizer(scale=3e-07)
...
def transformer_block(inputs, num_blocks, memory=None, mask=None, causality=False,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i in range(num_blocks):
            outputs = add_timing_signal_1d(outputs)
            outputs, sublayer, attention_weights = self_attention_block_trans(outputs, num_filters, seq_len,
                                                                              memory=memory,
                                                                              mask=mask, causality=causality,
                                                                              num_heads=num_heads,
                                                                              scope="self_attention_layers%d" % i,
...
def self_attention_block(inputs, num_filters, seq_len, memory=None, mask=None, causality=False, num_heads=8,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        outputs, attention_weights, _ = multihead_attention(outputs, num_filters, num_heads=num_heads,
                                                            memory=memory, seq_len=seq_len, causality=causality,
                                                            reuse=reuse,
...
def vanilla_attention(queries, units, num_heads,
    ...
    with tf.variable_scope(scope, default_name="vanilla_attention", reuse=reuse):
        v = tf.get_variable("v", units, regularizer=regularizer,
                            initializer=tf.zeros_initializer())
        b = tf.get_variable("b", units, regularizer=regularizer,
                            initializer=tf.zeros_initializer())
        ...
        coverage = tf.expand_dims(conv(attns, units, name="coverage_projection", reuse=reuse), 2)

        logits = tf.reduce_sum(v * tf.tanh(key + query + coverage + b), [-1])
        if mask is not None:
...
def bilinear_attention(queries, units, num_heads,
                       attns=None,
                       memory=None,
                       seq_len=None,
                       causality=False,
                       scope="Bilinear_Attention",
                       reuse=None,
                       mask=None,
                       return_weights=False,
                       bias=True,
    ...
    with tf.variable_scope(scope, default_name="bilinear_attention", reuse=reuse):
        # input projection
        memory_shapes = memory.shape.as_list()
        memory_tmp = tf.reshape(memory, [memory_shapes[0], -1, memory_shapes[-1]])
        key = conv(memory_tmp, units, name="memory_projection", reuse=reuse)
        key = tf.reshape(key, memory_shapes[:-1] + [units])

        if len(memory_shapes) == 4:
            queries = tf.expand_dims(queries, 2)
        # the product similarity between queries and keys, result in a similarity matrix
        logits = tf.matmul(queries, key, transpose_b=True)

        shapes = [x if x != None else -1 for x in logits.shape.as_list()]
        if mask is not None:
            # mask the similarity matrix
            mask = tf.cast(tf.reshape(mask, [-1, 1, 1, shapes[-1]]), tf.int32) if len(shapes) == 4 else \
                tf.cast(tf.reshape(mask, [-1, 1, shapes[-1]]), tf.int32)
            logits = mask_logits(logits, mask)
