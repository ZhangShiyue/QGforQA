...
from LIB.tf_utils.recurrent_layers import CudnnLstm, CudnnGru
...
def tri_linear_attention(x, keys, with_bias=False):
...
def bidaf_attention(context_h, query_h, joint_mask, sim_func,
                    output_alignment=False):
    '''
    ...
    output = tf.concat([context_h, select_query, context_h * select_query,
                        select_context * select_query], axis=2)
...
def nsmn_attention(context_h, query_h, joint_mask, sim_func,
                   output_alignment=False):
    '''
    slightly modified version of c. clark's bidaf code
    ...
    select_query = tf.matmul(query_probs, query_h)  # (batch, context_words, q_dim)
    # context output
    context_output = tf.concat([context_h, select_query, context_h - select_query, context_h * select_query], axis=-1)

    # probability of each context_word per query_word
    context_probs = tf.nn.softmax(tf.transpose(dist_matrix, [0, 2, 1]))
    # Batch matrix multiplication to get the attended vectors
    select_context = tf.matmul(context_probs, context_h)  # (batch, context_words, q_dim)
    # query output
    query_output = tf.concat([query_h, select_context, query_h - select_context, query_h * select_context], axis=-1)

    return context_output, query_output


def self_attention(context_h, joint_mask, sim_func, output_alignment=False):
    '''
    slightly modified version of c. clark's bidaf code
    ...
    dist_matrix = sim_func(context_h, context_h)
    ...
    # probability of each query_word per context_word
    # Batch matrix multiplication to get the attended vectors
    select_query = tf.matmul(query_probs, context_h)  # (batch, context_words, q_dim)
