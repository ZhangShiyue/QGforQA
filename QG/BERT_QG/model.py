...
class BertEmb(object):
    def __init__(self, config, seq_length, graph=None):
        """"embedded context using bert"""
        bert_config = modeling.BertConfig.from_json_file(config.bert_config_file)
        self.graph = graph if graph is not None else tf.Graph()
        with self.graph.as_default():
            self.input_ids = tf.placeholder(tf.int32, [config.batch_size, seq_length], name="input_ids")
            self.input_mask = tf.to_int32(tf.cast(self.input_ids, tf.bool))
            self.input_type_ids = tf.zeros((config.batch_size, seq_length), dtype=tf.int32)
            self.input_len = tf.reduce_sum(self.input_mask, axis=-1)
            ...
            bert_emb = all_layers[-1] + all_layers[-2] + all_layers[-3] + all_layers[-4]
            _, self.bert_emb, _ = tf.split(bert_emb, [1, seq_length - 2, 1], 1)


class QGModel(object):
    def __init__(self, config, word_mat=None, label_mat=None, pos_mat=None, ner_mat=None, trainable=True):
        self.config = config
        self.N = config.batch_size
        self.PL = config.para_limit
        self.QL = config.ques_limit
        self.d = config.hidden
        self.dw = config.word_dim
        ...
        self.bert_para = tf.placeholder(tf.float32, [self.N, self.PL, config.word_dim], "paragraph")
        ...
        plus_word_mat = tf.tile(tf.nn.embedding_lookup(word_mat, [100]), [self.PL, 1])
        self.plus_word_mat = tf.concat([word_mat, plus_word_mat], axis=0)
    ...
    def add_train_op(self):
        # add train op
        self.lr = tf.minimum(self.config.ml_learning_rate, 0.001 / tf.log(999.) *
    ...
    def build_graph(self):
        para_emb = self.input_embedding(self.bert_para, self.labels, self.pos_tags, self.ner_tags)
    ...
    def input_embedding(self, bert_rep, labels, pos_tags, ner_tags):
        ...
        with tf.variable_scope("Input_Embedding_Layer"):
            ...
            sent_emb = tf.concat([bert_rep, sent_label_emb, sent_pos_emb, sent_ner_emb], axis=-1)
            return sent_emb
    ...
    def search(self, beam_size, prev_probs=None):
        ...
        with tf.variable_scope("Decoder_Layer", reuse=True):
            ...
            finished = tf.cast(tf.zeros((self.N, 1)), tf.bool)   # whether [SEP] is encountered
    ...
    def _loop_function_search(self, beam_size, prev, attn_w, prev_probs, finished, i):
        ...
        plus_dist_g = tf.zeros([self.N, dim, self.PL]) + 1e-10
        dist_g = tf.concat([dist_g, plus_dist_g], axis=-1)
        ...
        # check if [SEP] was sampled, note that '102' should be the vocab id of [SEP]
        finished = tf.logical_or(tf.gather_nd(finished, index), tf.equal(prev_symbol, 102))
        # embedding_lookup
        emb_prev = tf.nn.embedding_lookup(self.plus_word_mat, prev_symbol)
...
class QGRLModel(QGModel):
    def __init__(self, config, word_mat=None, label_mat=None, pos_mat=None, ner_mat=None, trainable=True):
        QGModel.__init__(self, config, word_mat=word_mat, label_mat=label_mat,
